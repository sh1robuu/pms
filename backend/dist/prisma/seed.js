...
async function seed() {
    ...
    for (let floor = 1; floor <= 5; floor++) {
        for (let i = 1; i <= 10; i++) {
            ...
            let outOfOrderReason = null;
            if (roomNum === '203') {
                status = 'OUT_OF_ORDER';
                outOfOrderReason = 'Bathroom renovation';
            }
            else if (roomNum === '401') {
                status = 'VACANT_DIRTY';
    ...
    for (let floor = 1; floor <= 5; floor++) {
        const r5 = rooms.find((r) => r.roomNumber === floor.toString() + '05');
        const r6 = rooms.find((r) => r.roomNumber === floor.toString() + '06');
        if (r5 && r6) {
    ...
    const res1 = await prisma.reservation.create({
        data: {
            ...
            guestId: guests[0].id,
            assignedRoomId: rooms.find((r) => r.roomNumber === '301')?.id,
            arrivalDate: today,
    ...
    const res2 = await prisma.reservation.create({
        data: {
            ...
            guestId: guests[2].id,
            assignedRoomId: rooms.find((r) => r.roomNumber === '102')?.id,
            arrivalDate: today,
    ...
    const res5 = await prisma.reservation.create({
        data: {
            ...
            guestId: guests[4].id,
            assignedRoomId: rooms.find((r) => r.roomNumber === '105')?.id,
            arrivalDate: today,
            departureDate: threeDays,
            status: 'RESERVED',
            isConnecting: true,
    ...
    const res6 = await prisma.reservation.create({
        data: {
            ...
            guestId: guests[5].id,
            assignedRoomId: rooms.find((r) => r.roomNumber === '106')?.id,
            arrivalDate: today,
            departureDate: threeDays,
            status: 'RESERVED',
            isConnecting: true,
    ...
    const room302 = rooms.find((r) => r.roomNumber === '302');
    ...
    const room509 = rooms.find((r) => r.roomNumber === '509');
    ...
    await prisma.reservation.create({
        data: {
            ...
            assignedRoomId: rooms.find((r) => r.roomNumber === '103')?.id,
    ...
    await prisma.alert.create({ data: { type: 'CONNECTING_ROOM_CHECK', severity: 'MEDIUM', message: 'Connecting rooms 105-106 need inspection before VIP arrival', roomId: rooms.find((r) => r.roomNumber === '105')?.id } });
    ...
    await prisma.auditLog.createMany({
        data: [
            ...
            { action: 'ROOM_STATUS_CHANGED', entityType: 'Room', entityId: rooms.find((r) => r.roomNumber === '203')?.id || '', userId: manager.id, details: '{"from":"VACANT_CLEAN","to":"OUT_OF_ORDER"}' },
        ],
    });
